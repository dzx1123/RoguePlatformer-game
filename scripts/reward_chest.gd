...
func setup(gold_reward: int, heal_reward: int, interaction_key: String = "E") -> void:
...
func _create_prompt_bubble() -> void:
	...
	_prompt_style.border_color = Color(0.96, 0.70, 0.28, 0.88)
	...
	pointer.color = Color(0.96, 0.70, 0.28, 0.92)
	...
	_prompt_key_style.border_color = Color(0.98, 0.78, 0.38, 0.94)
...
func set_opener_position(opener_position: Vector2) -> void:
	...
	if is_instance_valid(_prompt_text_label):
		_prompt_text_label.text = "现在开启" if in_range else "开启宝箱"
...
func force_open() -> bool:
	...
	if is_instance_valid(_prompt_text_label):
		_prompt_text_label.text = "金币 +%d  生命 +%d" % [_gold_reward, _heal_reward]
		_prompt_text_label.add_theme_font_size_override("font_size", 13)
	if _prompt_style != null:
		_prompt_style.bg_color = Color(0.055, 0.12, 0.10, 0.98)
