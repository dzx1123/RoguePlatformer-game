...
func run_test() -> void:
	...
	for weapon in WeaponCatalog.all_weapon_ids():
		scene._load_layout(10)
		scene.player.configure_weapon(weapon)
		await create_timer(0.35).timeout
		assert(scene.beetle.is_on_floor(), "Beetle must stand on the teaching floor")
		assert(absf(scene.beetle.position.y - 592) < 12)
		...
		scene.player.position = Vector2(705, 592)
		Input.action_press(&"move_right")
		await physics_frame
		Input.action_release(&"move_right")
		var target = scene.beetle
	...
	scene.queue_free()
	await process_frame
	print("chapter2_beetle_combat_smoke: PASS")
	quit()
